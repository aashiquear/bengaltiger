...
#include "aUThermalExpansionEigenstrain.h"
#include "RankTwoTensor.h"
#include "EulerAngleProvider.h"
#include "RotationTensor.h"

registerMooseObject("bengaltigerApp", aUThermalExpansionEigenstrain);

template <>
InputParameters
validParams<aUThermalExpansionEigenstrain>()
{
...
aUThermalExpansionEigenstrain::aUThermalExpansionEigenstrain(const InputParameters & parameters)
...
void
aUThermalExpansionEigenstrain::computeQpEigenstrain()
{
  if (_ndisp > 2)
    mooseError("aUThermalExpansionEigenstrain is only to be used for 2 dimensional problems.");
  ...
  {
    if (_j == 0)
      theta0 = theta1 * I1 + theta1 * I2;
    else if (_j == 1)
      theta0 = theta1 * I1 + theta2 * I2;
    else
      theta0 = theta1 * I1 + theta3 * I2;
  }
  else if (_i == 1)
  {
    if (_j == 0)
      theta0 = theta2 * I1 + theta1 * I2;
    else if (_j == 1)
      theta0 = theta2 * I1 + theta2 * I2;
    else
      theta0 = theta2 * I1 + theta3 * I2;
  }
  else
  {
    if (_j == 0)
      theta0 = theta3 * I1 + theta1 * I2;
    else if (_j == 1)
      theta0 = theta3 * I1 + theta2 * I2;
    else
      theta0 = theta3 * I1 + theta3 * I2;
  }
  ...
  {
    if (_j == 0)
      dtheta_dt0 = dtheta1_dt * I1 + dtheta1_dt * I2;
    else if (_j == 1)
      dtheta_dt0 = dtheta1_dt * I1 + dtheta2_dt * I2;
    else
      dtheta_dt0 = dtheta1_dt * I1 + dtheta3_dt * I2;
  }
  else if (_i == 1)
  {
    if (_j == 0)
      dtheta_dt0 = dtheta2_dt * I1 + dtheta1_dt * I2;
    else if (_j == 1)
      dtheta_dt0 = dtheta2_dt * I1 + dtheta2_dt * I2;
    else
      dtheta_dt0 = dtheta2_dt * I1 + dtheta3_dt * I2;
  }
  else
  {
    if (_j == 0)
      dtheta_dt0 = dtheta3_dt * I1 + dtheta1_dt * I2;
    else if (_j == 1)
      dtheta_dt0 = dtheta3_dt * I1 + dtheta2_dt * I2;
    else
      dtheta_dt0 = dtheta3_dt * I1 + dtheta3_dt * I2;
  }
  ...
  for (auto op_index = beginIndex(op_to_grains); op_index < op_to_grains.size(); ++op_index)
  {
    ...
    if (grain_id < _euler.getGrainNum())
      ...
      mooseError("aUThermalExpansionEigenstrain has run out of grain rotation data.");
