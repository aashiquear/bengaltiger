...
#include "PolycrystalElasticityTensor2.h"
#include "EulerAngleProvider.h"
#include "RotationTensor.h"

registerMooseObject("bengaltigerApp", PolycrystalElasticityTensor2);

template <>
InputParameters
validParams<PolycrystalElasticityTensor2>()
{
...
PolycrystalElasticityTensor2::PolycrystalElasticityTensor2(const InputParameters & parameters)
...
void
PolycrystalElasticityTensor2::computeQpElasticityTensor()
{
  ...
  for (auto op_index = beginIndex(op_to_grains); op_index < op_to_grains.size(); ++op_index)
  {
    ...
    if (grain_id < _euler.getGrainNum())
      ...
      mooseError("PolycrystalElasticityTensor2 has run out of grain rotation data.");
