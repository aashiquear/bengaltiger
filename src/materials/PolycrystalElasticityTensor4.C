...
#include "PolycrystalElasticityTensor4.h"
#include "EulerAngleProvider.h"
#include "RotationTensor.h"

registerMooseObject("bengaltigerApp", PolycrystalElasticityTensor4);

template <>
InputParameters
validParams<PolycrystalElasticityTensor4>()
{
...
PolycrystalElasticityTensor4::PolycrystalElasticityTensor4(const InputParameters & parameters)
...
void
PolycrystalElasticityTensor4::computeQpElasticityTensor()
{
  ...
  for (auto op_index = beginIndex(op_to_grains); op_index < op_to_grains.size(); ++op_index)
  {
    ...
    if (grain_id < _euler.getGrainNum())
      ...
      mooseError("PolycrystalElasticityTensor4 has run out of grain rotation data.");
