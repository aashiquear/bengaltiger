...
#include "PolycrystalBurnupEigenstrain4.h"
#include "EulerAngleProvider.h"
#include "RotationTensor.h"
#include "RankTwoScalarTools.h"

registerMooseObject("bengaltigerApp", PolycrystalBurnupEigenstrain4);

template <>
InputParameters
validParams<PolycrystalBurnupEigenstrain4>()
{
...
PolycrystalBurnupEigenstrain4::PolycrystalBurnupEigenstrain4(const InputParameters & parameters)
...
void
PolycrystalBurnupEigenstrain4::computeQpEigenstrain()
{
  ...
  for (auto op_index = beginIndex(op_to_grains); op_index < op_to_grains.size(); ++op_index)
  {
    ...
    if (grain_id < _euler.getGrainNum())
      ...
      mooseError("PolycrystalBurnupEigenstrain4 has run out of grain rotation data.");
    ...
    Real h = (1 + std::tanh(libMesh::pi * ((n - 0.5) * 9 / 2))) / 2;
