...
#include "PolycrystalElasticityTensor3.h"
#include "EulerAngleProvider.h"
#include "RotationTensor.h"

registerMooseObject("bengaltigerApp", PolycrystalElasticityTensor3);

template <>
InputParameters
validParams<PolycrystalElasticityTensor3>()
{
...
PolycrystalElasticityTensor3::PolycrystalElasticityTensor3(const InputParameters & parameters)
...
void
PolycrystalElasticityTensor3::computeQpElasticityTensor()
{
  ...
  for (auto op_index = beginIndex(op_to_grains); op_index < op_to_grains.size(); ++op_index)
  {
    ...
    if (grain_id < _euler.getGrainNum())
      ...
      mooseError("PolycrystalElasticityTensor3 has run out of grain rotation data.");
