...
#include "PolycrystalBurnupEigenstrain3.h"
#include "EulerAngleProvider.h"
#include "RotationTensor.h"
#include "RankTwoScalarTools.h"

registerMooseObject("bengaltigerApp", PolycrystalBurnupEigenstrain3);

template <>
InputParameters
validParams<PolycrystalBurnupEigenstrain3>()
{
...
PolycrystalBurnupEigenstrain3::PolycrystalBurnupEigenstrain3(const InputParameters & parameters)
...
void
PolycrystalBurnupEigenstrain3::computeQpEigenstrain()
{
  ...
  for (auto op_index = beginIndex(op_to_grains); op_index < op_to_grains.size(); ++op_index)
  {
    ...
    if (grain_id < _euler.getGrainNum())
      ...
      mooseError("PolycrystalBurnupEigenstrain3 has run out of grain rotation data.");
    ...
    Real h = (1 + std::tanh(libMesh::pi * ((n - 0.5) * 5 / 2))) / 2;
