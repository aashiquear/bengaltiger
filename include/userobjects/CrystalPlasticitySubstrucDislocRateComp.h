...
class CrystalPlasticitySubstrucDislocRateComp;

/**
 * Phenomenological constitutive model state variable evolution rate component userobject class.
 */
class CrystalPlasticitySubstrucDislocRateComp : public CrystalPlasticityStateVarRateComponent
{
public:
  static InputParameters validParams();

  CrystalPlasticitySubstrucDislocRateComp(const InputParameters & parameters);
...
protected:
  ...
  /// Dislocation recovery rate coefficient for each slip system (see Knezevic et al. 2012)
  DenseVector<Real> _f;

  /// Rate coefficient for each slip system (see Knezevic et al. 2012)
  DenseVector<Real> _q;
